...
- **长录音（≥ 5分钟）**：逐字稿精读 → 提取待办/素材/金句/人脉/踩坑/方案
- **低价值**：静默归档
...
| sk-info-assets skill | 长录音 8 节信息资产分析 | 可选（无则长录音用内置精读规则） |
...
- 状态文件默认在 skill 目录下的 `status.md`
- 金句存档默认在 skill 目录下的 `output/daily-quotes/`
- 可在 `rules.md` 中通过 `WORKSPACE_DIR` 自定义输出目录
